...
// паттерн адаптер позволяет интегрировать легаси код с новыми системами
// без изменения существующего кода, обеспечивая совместимость между
// несовместимыми интерфейсами.
// используется например в миграциях между версиями API
// плюсы - переиспользование существующего кода, снижение затрат на разработку, разделение ответственности
// минусы - может добавлять накладные расходы, усложнять архитектуру
func main() {
	oldLogger := &OldLogger{}
